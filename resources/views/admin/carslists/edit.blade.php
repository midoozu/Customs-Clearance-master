...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            {{ trans('global.edit') }} {{ trans('cruds.carslist.title_singular') }}
        </div>

        <div class="card-body">
            <form method="POST" action="{{ route("admin.carslists.update", [$carslist->id]) }}" enctype="multipart/form-data">
                @method('PUT')
                @csrf
                <div class="form-group">
                    <label class="required" for="car_type">{{ trans('cruds.carslist.fields.car_type') }}</label>
                    <input class="form-control {{ $errors->has('car_type') ? 'is-invalid' : '' }}" type="text" name="car_type" id="car_type" value="{{ old('car_type', $carslist->car_type) }}" required>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('car_plate') ? 'is-invalid' : '' }}" type="text" name="car_plate" id="car_plate" value="{{ old('car_plate', $carslist->car_plate) }}" required>
